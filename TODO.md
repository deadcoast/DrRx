...
1) Token Spec and Scopes: COMPLETE (VS Code)
   ...
   - Normative scope mapping captured in `drrx/syntax-overview.md`; keep updated when scopes change.
2) Grammar/Highlighter (VS Code): PARTIAL
   - Grammar covers operators, flow prefixes, spacers, names, annotations, comments.
   - Needs profiling on very large fixtures and potential optimisations for misaligned `|` cases currently caught by diagnostics.
3) Diagnostics Engine (VS Code): PARTIAL
   ...
   - Quick-fixes: FW.06 (insert spacer), OR.02 (regroup), SP.05 (normalize spacing), VL.07 (remove spacer), FW.02/SP.03 (insert `|`). Removal/normalisation passes still TODO.
4) Fixtures and Tests: COMPLETE
   - Diagnostics and token goldens checked into `test/golden/` and `test/token-golden/`; perf harness documented in `test/perf/README.md`.
5) Tooling and CI: PARTIAL
   - `npm test` and schema validation run in CI; VSIX packaged on tags. Marketplace publishing and stricter PR gating remain.
6) Documentation Sync: PARTIAL
   - README includes highlighter guidance; continue aligning `drrx/udl-validation-map.md` and examples with engine changes.
...
- Diagnostics Engine
  - Continuity removal quick fix (VL.02) added; follow up with normalisation flows and cover remaining warnings (FW.03, FW.11) with actionable edits.
- Fixtures and Tests
  - Expand regression coverage for mixed warning/error scenarios before broadening rule enforcement.
- Tooling and CI
  - Tighten CI (fail on golden drift, upload diagnostics artefacts) and prep marketplace publishing workflow when feature-complete.
- Documentation Sync
...
- Tokenization goldens maintained; perf baseline refreshed when diagnostics change.
