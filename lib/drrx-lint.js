...
function tokenizeLine(line) {
  const tokens = [];
  const addToken = (kind, start, end) => {
    if (start >= end) return;
    tokens.push({ kind, start, end, text: line.slice(start, end) });
  };
  const commentIdx = findCommentIndex(line);
  const codeEnd = commentIdx === -1 ? line.length : commentIdx;
  let cursor = 0;

  // leading indentation and tabs
  let indentSpaces = 0;
  while (cursor < codeEnd) {
    ...
    if (ch === '\t') {
      addToken('invalid.tab', cursor, cursor + 1);
      cursor++;
      continue;
    }
    break;
  ...
  if (indentSpaces > 0) {
    const even = indentSpaces - (indentSpaces % 2);
    if (even) addToken('indent', cursor - indentSpaces, cursor - indentSpaces + even);
    if (indentSpaces % 2) addToken('indent.misalign', cursor - 1, cursor);
  }

  // flow prefix tokens (allow multiple markers)
  while (cursor < codeEnd) {
    const ch = line[cursor];
    if (ch === '+' || ch === ':') {
      addToken(ch === '+' ? 'flow.open' : 'flow.close', cursor, cursor + 1);
      cursor++;
      cursor = consumeSpaces(line, cursor, codeEnd, addToken, 'prefix.gap');
      continue;
    }
    if (ch === '|') {
      addToken('flow.cont', cursor, cursor + 1);
      cursor++;
      cursor = consumeSpaces(line, cursor, codeEnd, addToken, 'prefix.gap');
      continue;
    }
  ...
  if (cursor < codeEnd && line[cursor] === '.' && line.slice(cursor + 1, codeEnd).trim() === '') {
    addToken('root', cursor, cursor + 1);
    cursor = codeEnd;
  } else {
    // operator tokens
    if (cursor + 2 <= codeEnd) {
      const op = line.slice(cursor, cursor + 2);
      if (op === '--' || op === '==') {
        addToken(op === '--' ? 'operator.dir' : 'operator.file', cursor, cursor + 2);
        cursor += 2;
        cursor = consumeOperatorGap(line, cursor, codeEnd, addToken);
      }
    }

    // name token (quoted or unquoted)
    if (cursor < codeEnd) {
      if (line[cursor] === '"') {
        let end = cursor + 1;
        let inEscape = false;
        while (end < codeEnd) {
          const ch = line[end];
          if (ch === '"' && !inEscape) { end++; break; }
          inEscape = ch === '\\' ? !inEscape : false;
          end++;
        }
        addToken('name.quoted', cursor, Math.min(end, codeEnd));
        cursor = Math.min(end, codeEnd);
      } else {
        const nameMatch = line.slice(cursor, codeEnd).match(/^[^ \t{]+/);
        if (nameMatch) {
          addToken('name.unquoted', cursor, cursor + nameMatch[0].length);
          cursor += nameMatch[0].length;
        }
      }
      cursor = consumeSpaces(line, cursor, codeEnd, addToken, 'name.gap');
    }

    // annotations
    while (cursor < codeEnd && line[cursor] === '{') {
      let end = cursor + 1;
      let inQuote = false;
      while (end < line.length) {
        const ch = line[end];
        if (ch === '"') {
          let bs = 0; let j = end - 1;
          while (j >= 0 && line[j] === '\\') { bs++; j--; }
          if (bs % 2 === 0) inQuote = !inQuote;
        }
        if (!inQuote && ch === '}') { end++; break; }
        end++;
      }
      addToken('annotation.block', cursor, Math.min(end, line.length));
      cursor = Math.min(end, codeEnd);
      cursor = consumeSpaces(line, cursor, codeEnd, addToken, 'name.gap');
    }

    if (cursor < codeEnd) {
      cursor = consumeSpaces(line, cursor, codeEnd, addToken, 'trailing.whitespace');
    }

    if (cursor < codeEnd) {
      addToken('unknown', cursor, codeEnd);
      cursor = codeEnd;
    }
  }

  if (commentIdx !== -1) {
    addToken('comment', commentIdx, line.length);
  } else if (codeEnd < line.length) {
    addToken('trailing.whitespace', codeEnd, line.length);
  }

  return tokens;
}

function consumeSpaces(line, cursor, end, addToken, kind) {
  let start = cursor;
  while (cursor < end && (line[cursor] === ' ' || line[cursor] === '\t')) {
    if (line[cursor] === '\t') {
      if (start < cursor) addToken(kind, start, cursor);
      addToken('invalid.tab', cursor, cursor + 1);
      cursor++;
      start = cursor;
    } else {
...
function consumeOperatorGap(line, cursor, end, addToken) {
  const start = cursor;
  let spaces = 0;
  let hasTab = false;
  while (cursor < end && (line[cursor] === ' ' || line[cursor] === '\t')) {
  ...
  if (!hasTab && spaces === 1) {
    addToken('operator.space', start, cursor);
  } else {
    addToken('invalid.spacing.afterOperator', start, cursor);
  }
  return cursor;
}

module.exports = { lintText, tokenizeText };
